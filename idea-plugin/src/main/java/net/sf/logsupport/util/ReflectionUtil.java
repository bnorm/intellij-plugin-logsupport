...
public class ReflectionUtil {
	...
	private static synchronized Method findMethod(Class<?> cls, String methodName, Class... paramTypes) {
		...
		key.add(cls);
		key.add(methodName);
		Collections.addAll(key, paramTypes);

		Method method = methods.get(key);
		if (method == null) {
			Method[] methodList = cls.getMethods();

			// Look after a public method that matches the signature
			method = doFindMethod(methodName, methodList, paramTypes);

			// Look after in-accessible methods if public methods didn't match.
			for (Class<?> c = cls; method == null && c != null; c = c.getSuperclass()) {
	...
	private static Method doFindMethod(String methodName, Method[] methods, Class[] paramTypes) {
		search:
